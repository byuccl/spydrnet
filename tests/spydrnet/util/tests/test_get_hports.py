...
class TestGetHPorts(unittest.TestCase):
    ...
    def test_get_hpin_from_href_pin(self):
        ...
        href = next(pin.get_hports())
        href_from_href = next(sdn.get_hports(href))
        self.assertIs(href, href_from_href)

    def test_get_hports_from_hrefs_of_cable_and_wire(self):
        library = self.netlist.libraries[1]
        definition = library.definitions[0]
        cable = definition.cables[0]
        wire = cable.wires[0]
        ...
        href_cable_pins = set(sdn.get_hports(cable_href))
        href_wire_pins = set(sdn.get_hports(wire_href))
        self.assertTrue(len(href_cable_pins) > 0 and len(href_wire_pins) > 0)
        self.assertTrue(all(isinstance(x.item, sdn.Port) for x in href_cable_pins))
        self.assertTrue(all(isinstance(x.item, sdn.Port) for x in href_wire_pins))
        self.assertTrue(all(x in href_cable_pins for x in href_wire_pins))

    def test_get_hports_from_hrefs_of_port_and_pin(self):
        library = self.netlist.libraries[1]
        definition = library.definitions[0]
        port = definition.ports[0]
        pin = port.pins[0]
        ...
        self.assertTrue(href_result is port_href)
        href_result = next(sdn.get_hports(pin_href), None)
        self.assertTrue(href_result is port_href)
    ...
    def test_bad_selection_type(self):
        ...
        self.assertRaises(TypeError, self.netlist.get_hports, selection=None)

    def test_of_bad_instance(self):
        hrefs = list(sdn.get_hports(sdn.Instance()))
        self.assertTrue(len(hrefs) == 0)
    ...
    def test_through_hierarchy_again(self):
        ...
        hrefs = set(sdn.get_hports(middle_cable))
        ...
        all_pins = set(netlist.get_hports(recursive=True))
