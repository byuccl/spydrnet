...
class Composer:
    ...
    def _write_from_top(self, instance):
        ...
        while(len(to_write) != 0):
            ...
            assert definition.name is not None, self._error_string("definition has no name set", definition)
            self._write_module(definition)
    ...
    def _write_assignment(self, instance):
        ...
        for p in instance.reference.ports:
            ...
            if p.name == "i":
                in_port = p
        assert in_port is not None, self._error_string("instance does not appear to be an assignment with a port named i and o", instance)
        assert out_port is not None, self._error_string("instance does not appear to be an assignment with a port named i and o", instance)
        left_wires = []
        right_wires = []
        in_pins = []
        out_pins = []
        ...
        assert not self._is_pinset_concatenated(in_pins, in_wires[0].cable.name), self._error_string("multiple cables appear to be connected to a single assignment input", instance)
        assert not self._is_pinset_concatenated(out_pins, out_wires[0].cable.name), self._error_string("multiple cables appear to be connected to a single assignment output", instance)
    ...
    def _write_name(self, object):
        ...
        assert object.name is not None, self._error_string("name of object is not set", object)
        if object.name[0] == '\\':
            assert object.name[-1] == ' ', self._error_string("the object name starts with escape and does not end with a space.", object)
    ...
    def _write_brackets_defining(self, bundle):
        ...
        assert width != 0, self._error_string("bundle has 0 width, this will not write correctly please add a pin to the port or wire to the cable.", bundle)
    ...
    def _write_brackets(self, bundle, low_index, high_index):
        ...
        if low_index == lower_bundle and high_index == upper_bundle:
            return #no need to write if the upper and lower indicies cover the whole bundle.
            ...
            assert low_index is not None and high_index is not None,\
                self._error_string("low_index and high_index should both not be None at this point in the function")
