...
class Composer:
    ...
    def _write_from_top(self, instance):
        ...
        while(len(to_write) != 0):
            ...
            if definition.name is None:
                raise Exception("Definition: ", definition,
                                " .name is undefined")
            self._write_definition_single(definition)
    ...
    def _write_assignments(self, instance):
        cable1_name, cable2_name, left_low, left_high, right_low, right_high = self._get_assignment_indicies(
            instance)
    ...
    def _write_port_wires(self, pins):
        ...
        def terminate_slice():
            ...
            if not single_slice:
                string_to_write += ", "
            string_to_write += self._get_indexed_name_from_cable(
                previous_cable, low_index, high_index, counting_down)
            single_slice = False
            low_index = None
            high_index = None
            counting_down = None
        ...
        for pin in pins:
            if pin.wire == None:
                ...
                wire_exists = True
                assert last_wire == False, "there is a gap in the port's pins to be wired up " + \
        ...
        if single_slice:
            ...
            string_to_write = self._get_indexed_name_from_cable(
                cable, low_index, high_index, counting_down)
        else:
            string_to_write += ", " + \
                self._get_indexed_name_from_cable(
                    cable, low_index, high_index, counting_down)
            string_to_write += " }"
