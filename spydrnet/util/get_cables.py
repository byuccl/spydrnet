...
def get_cables(obj, *args, **kwargs):
    ...
    return _get_cables(object_collection, patterns, key, is_case, is_re, filter_func)


def _get_cables(object_collection, patterns, key, is_case, is_re, filter_func):
    unique_results = set()
    for result in filter(filter_func, _get_cables_raw(object_collection, patterns, key, is_case, is_re)):
...
def _get_cables_raw(object_collection, patterns, key, is_case, is_re):
    for pattern in patterns:
        ...
        for parent in _get_cable_parents(object_collection):
            if pattern_is_absolute:
                result = lookup(parent, Cable, key, pattern)
