from spydrnet import FirstClassElement, InnerPin, OuterPin, Wire, Netlist, Library, Definition, Instance
...
def get_instances(obj, *args, **kwargs):
    ...
    return _get_instances(object_collection, patterns, key, is_case, is_re, filter_func)


def _get_instances(object_collection, patterns, key, is_case, is_re, filter_func):
    unique_results = set()
    for result in filter(filter_func, _get_instances_raw(object_collection, patterns, key, is_case, is_re)):
        if result not in unique_results:
            unique_results.add(result)
            yield result


def _get_instances_raw(object_collection, patterns, key, is_case, is_re):
    for pattern in patterns:
        pattern_is_absolute = _is_pattern_absolute(pattern, is_case, is_re)
        for parent in _get_instance_parents(object_collection):
            if pattern_is_absolute:
                result = lookup(parent, Instance, key, pattern)
...
def _get_instance_parents(object_collection):
